    ...
    class Pagination < Generator
      ...
      def generate(site)
        if Pager.pagination_enabled?(site)
          path_array = site.config['paginate_path']
      ...
      def paginate(site, page)
        ...
        name = site.config['paginate_path'].split('/')[0] # lj modify: len-2->0
        if name != nil
          all_posts = all_posts.find_all { |p| p['category'] == name } #lj modify:ascription->category
        else
